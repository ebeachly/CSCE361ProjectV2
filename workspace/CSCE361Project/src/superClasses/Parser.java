...
public class Parser {
	...
	public boolean parseInputAndExecute(String input){
		//parses input and executes it. Returns false if the input was not understood.
		...
		//These indices represent the range that the part of speech could exist in.
		...
		int currentPartOfSpeechBeingSearchedFor = 0;	//0: None, 1: Object, 2: Action, 3: Target
		
		//Iterate through up to the second to last word, because if the preposition is the last word, then we can't say that there is a part of speech after it.
		for( int i = 0; i < words.length - 1; ++i ){
			
			//Start defining the range the part of speech is in, given what the preposition is.
			if( words[i].equals("with") || words[i].equals("use")){
				...
				//We can assume the Action will be in the first section if the first word was not a preposition
				currentPartOfSpeechBeingSearchedFor = 2;
		...
		System.out.println("DEBUG: action is inclusively between words " + startIndexOfAction + " and " + endIndexOfAction);
		System.out.println("DEBUG: object is inclusively between words " + startIndexOfObject + " and " + endIndexOfObject);
		System.out.println("DEBUG: target is inclusively between words " + startIndexOfTarget + " and " + endIndexOfTarget);
		//So we have defined ranges of where we think the parts of speech are.
		//Now we need to find the parts of speech within those ranges by checking for matches with our Thesaurus
		
		String action = "";
		String object = "";
		String target = "";
		
		//The action is the most important word to find. Search looking for a match for that first.
		
		//We will search within the range by first chopping off words from the end. If no matches arise, then we'll chop off a value from the beginning and repeat.
		boolean match = false;
		while( startIndexOfAction < endIndexOfAction ){
			...
			//If no match for all patterns that start with this word, increase the start index and reset the end index
			++startIndexOfAction;
		}
		if(!match){
		...
		//Update the start or end indices of the other parts of speech if they fall within what we have identified as the action
		if( startIndexOfObject >= startIndexOfAction && startIndexOfObject <= endIndexOfAction && endIndexOfAction != words.length){
			startIndexOfObject = endIndexOfAction + 1;
		}
		if( endIndexOfObject >= startIndexOfAction && endIndexOfObject <= endIndexOfAction && startIndexOfAction != 0){
			endIndexOfObject = startIndexOfAction - 1;
		}
		if( startIndexOfTarget >= startIndexOfAction && startIndexOfTarget <= endIndexOfAction && endIndexOfAction != words.length){
			startIndexOfTarget = endIndexOfAction + 1;
		}
		if( endIndexOfTarget >= startIndexOfAction && endIndexOfTarget <= endIndexOfAction && startIndexOfAction != 0){
		...
		while( startIndexOfObject <= endIndexOfObject ){
			...
			while( startIndexOfObject <= tempEndIndex){
				...
				//If we're looking inside what we've identified as the action, no point
				if( startIndexOfObject >= startIndexOfAction && tempEndIndex <= endIndexOfAction ){
					break;
				}
		...
		//Update the target indices based off what we have learned about the object.
		if( startIndexOfTarget >= startIndexOfObject && startIndexOfTarget <= endIndexOfObject && endIndexOfObject != words.length){
			startIndexOfTarget = endIndexOfObject + 1;
		}
		if( endIndexOfTarget >= startIndexOfObject && endIndexOfTarget <= endIndexOfObject && startIndexOfObject != 0){
		...
		while( startIndexOfTarget <= endIndexOfTarget ){
			...
			while( startIndexOfTarget <= tempEndIndex){
				...
				//If we're looking inside what we've identified as the action, or the object, no point
		...
		//Now we have found the action, the object, and maybe the target. Time to execute the command
		...
		if( action.equals("quit") ){
			...
			Game.player.pickUp(object);	//pickup needs to find the object and remove it, so if we found the pointer, we would just have to search again anyways because we still don't know where it is.
			return true;
		} else {
			...
			if( objectPointer == null ){
				System.out.println("SYSTEM: Could not recognize a direct object in your input.");
				return false;
			}
	...
	private Interactable findInteractable(String name){
		//Searches inventory and aspect list of the current location for a match to name.
		...
		for( Interactable i : Game.player.inventory ){
			if( i.name.equals(name) ){
				...
				//If it has the same name as something already in the list, don't add it
				boolean unique = true;
		...
		if( potentialMatches.isEmpty() ){
			...
			//Ambiguity
			System.out.println("SYSTEM: Could not determine what you meant by " + name + ". Please rephrase your input using one of these:");
