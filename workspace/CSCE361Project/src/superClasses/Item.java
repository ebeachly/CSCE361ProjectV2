...
public abstract class Item extends Interactable {
	...
	protected boolean drop() {
		...
		// Pick-up function in Player class because you can't interact with
		// items on the ground.
		if (Game.player.dropItem(this)) {
			System.out
					.println("You drop your " + this.name + " on the ground.");
			return true;
		}
		return false;
	}
	...
	protected boolean toss(Interactable target) {
		if (Game.player.dropItem(this)) {
			if (target != null) {
				System.out.println("You throw your " + this.name + " at the "
						+ target.name + ".");
				if (!target.damage(this)) {
					System.out.println("The " + this.name
							+ " bounces off and falls on the ground.");
				}
			} else {
				System.out.println("You throw your " + this.name
						+ " away, and it lands on the ground.");
			}
			return true;
		}
		return false;
